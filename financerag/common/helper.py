...
def timer(func):
    ...
    def measure_time(*args, **kwargs):
        ...
        if "retrieve" in func.__name__:
            ...
            logger.info(
                f"Retrieval time for {query_len} queries, {top_k} docs/query = {round(avg_time, 4)}s"
            )
            ...
            if corpus is not None:
                ...
                logger.info(
                    f"Loading {corpus_len} documents - Time taken for 1 doc = {avg_time}s"
                )
            else:
                logger.info(f"Loading documents = {total_time_taken}s")
        elif "rerank" in func.__name__:
            retrieved_result: Dict[str, Dict[str, float]] = kwargs.get(
            ...
            if isinstance(retrieved_result, Dict):
                ...
                avg_time = total_time_taken / query_len
                logger.info(
                    f"Reranking for {query_len} queries - Time taken for reranking {doc_len} docs/query = {avg_time}s"
                )
...
def get_query_and_retrieved_corpus_text(
...
def load_query(dataset_name: str, new_path_to_query=None) -> Dict[str, str]:
    query_df = pd.read_json(
        f"../../finance_dataset/{dataset_name.lower()}_queries.jsonl/queries.jsonl",
        lines=True,
    )
...
def load_corpus(dataset_name: str, new_path_to_corpus=None) -> Dict[str, str]:
    corpus_df = pd.read_json(
        f"finance_dataset/{dataset_name.lower()}_corpus.jsonl/corpus.jsonl", lines=True
    )
    # Convert into Dict[str, str]
...
def get_final_result(dataset_names: List[str], method_name: str):
    ...
    for dataset_name in dataset_names:
        df = pd.read_csv(
            f"financerag_result/{method_name}/{dataset_name.lower()}_result.csv"
        )
